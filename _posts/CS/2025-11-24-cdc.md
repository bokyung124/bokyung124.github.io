...
last_modified_at: 2025-11-24T05:17:00+00:00
...
CDC 도구가 로그에 접근할 수 있도록 소스 DB의 환경을 구성하는 단계입니다.

- **로깅 활성화:** DB의 설정 파일에서 변경 사항을 기록하는 옵션을 켭니다.
  - **MySQL:** binlog_format = ROW (SQL 문장이 아닌 변경된 데이터 값 자체를 기록하도록 설정)

  - **PostgreSQL:** wal_level = logical (외부에서 논리적으로 해석 가능한 로그 레벨로 설정)

- **계정 및 권한:** CDC 도구가 사용할 전용 DB 계정을 생성하고, 복제 프로토콜을 사용할 수 있는 권한(REPLICATION CLIENT, REPLICATION SLAVE)을 부여합니다.

### 2) CDC 커넥터 배포 및 초기 적재 (Initial Snapshot & Streaming)

CDC 엔진(예: Kafka Connect + Debezium)을 실행하여 소스 DB와 연결합니다.

- **핸드셰이크(Handshake):** CDC 커넥터가 소스 DB에 접속하여 자신을 '복제본(Replica)'으로 등록합니다.

- **초기 스냅샷(Snapshot):** 로그는 '연결 시점 이후'의 변경분만 존재합니다. 따라서 최초 실행 시에는 기존에 쌓여 있는 모든 데이터를 SELECT * 형태로 조회하여 타겟으로 먼저 넘깁니다.

- **로그 스트리밍(Log Streaming):** 스냅샷이 완료되면 자동으로 모드를 전환하여, 실시간으로 발생하는 트랜잭션 로그(Binlog/WAL)를 읽어들이기 시작합니다.

### 3) 메시지 변환 및 버퍼링 (Serialization & Buffering)

읽어들인 바이너리 데이터를 가공하여 메시지 큐에 저장합니다.

- **직렬화(Serialization):** DB 내부의 바이너리 데이터를 범용적인 포맷(JSON, Avro, Protobuf 등)으로 변환합니다. 이 과정에서 메타데이터(생성 시간, 트랜잭션 ID, 변경 전/후 값)가 포함된 구조체(Struct)가 생성됩니다.

- **토픽 라우팅(Topic Routing):** 일반적으로 서버명.데이터베이스명.테이블명 형식으로 Kafka 토픽을 구분하여 데이터를 발행(Publish)합니다. Kafka는 이 데이터를 디스크에 저장하여 타겟 시스템의 장애나 지연에 대비한 버퍼(Buffer) 역할을 수행합니다.

### 4) 타겟 시스템 반영 (Sink & Apply)

메시지 큐에 쌓인 데이터를 최종 목적지에 씁니다.

- **Sink Connector:** Kafka에 저장된 데이터를 읽어 타겟 DB(Snowflake, BigQuery, ElasticSearch 등)에 적재하는 역할을 합니다. 별도의 코딩 없이 설정(Configuration)만으로 동작하는 경우가 많습니다.

- **데이터 반영(Apply):**
  - **Upsert:** 소스에서의 INSERT와 UPDATE는 타겟에서 Upsert(없으면 입력, 있으면 수정) 로직으로 처리하여 데이터 정합성을 맞춥니다.

  - **Delete 처리:** 소스에서의 DELETE 이벤트는 타겟에서 실제 데이터를 삭제하거나, is_deleted 플래그를 업데이트하는 방식(Soft Delete)으로 처리합니다.
...
1. **오프셋 관리 (Offset Management):**
  - CDC 도구는 자신이 로그의 "어디까지 읽었는지"에 대한 위치 정보(Offset, LSN 등)를 주기적으로 저장합니다.

  - 시스템이 재시작되더라도 저장된 오프셋 위치부터 다시 읽기 시작하여 데이터 유실이나 중복을 방지(Exactly-once 또는 At-least-once)합니다.

2. **스키마 레지스트리 (Schema Registry):**
  - 소스 DB의 테이블 구조(컬럼 추가/변경)가 바뀌었을 때, CDC 도구는 이를 감지하여 버전(Version)을 관리합니다. 이를 통해 데이터 구조 변경 시 파이프라인이 중단되지 않도록 대응합니다.

3. **데이터 일관성 (Consistency):**
  - CDC는 트랜잭션의 순서를 보장해야 합니다. 따라서 Kafka의 파티션(Partition)을 설정할 때, 동일한 Primary Key를 가진 데이터는 반드시 동일한 파티션으로 들어가도록 설정하여 순서 뒤바뀜을 방지합니다.
